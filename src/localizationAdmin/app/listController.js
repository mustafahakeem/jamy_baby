...
(function(undefined) {
    ...
    listController.$inject = ['$scope', '$timeout', '$upload', 'localizationService', 'authService'];

    function listController($scope, $timeout, $upload,  localizationService, authService) {
        ...
        vm.onSaveResourceClick = function() {
            vm.updateResource(vm.activeResource)
                .success(function() {
                    ...
                    var resourceSet = vm.activeResource.ResourceSet ? vm.activeResource.ResourceSet : "Resources";
        ...
        vm.saveGridResource = function(resource) {
            localizationService.updateResource(resource)
                .success(function() { 
                    showMessage(vm.dbRes('ResourceSaved') +
                     " [" + resource.ResourceId + 
                     " (" + (resource.LocaleId ? resource.LocaleId : "text") + ")]");
                    ...
                    localizationService.getResourceSets()
                        .success(function(resourceSets) {
                            ...
                            for (var i = 0; i < resourceSets.length; i++) {
                                localizationService.deleteResourceSet(resourceSets[i])
                                    .success(function() {
                                        console.log("remove resourceSet");
                                    });
                            }
                        })
                        .error(parseError);
