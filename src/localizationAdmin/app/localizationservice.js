(function() {
    ...
    localizationService.$inject = ['$http', '$q', '$timeout', 'envService'];

    function localizationService($http, $q, $timeout, envService) {

        var service = {
            error: null,
            baseUrl: envService.read('apiUrl')+"/api/LocalizationAdministration/",
            blockbondsApi: envService.read('blockbondsApi'),
            jamesApi: envService.read('apiUrl'),
            identityServer: envService.read('identityServer'),
        ...
        function clearCache() {
            $http.get(this.blockbondsApi + "/api/SystemConfig/ClearUserMessageCache")
                .error(parseHttpError);

            $http.get(this.identityServer + "/api/SystemConfig/ClearUserMessageCache")
                .error(parseHttpError);

            $http.get(this.jamesApi + "/api/SystemConfig/ClearUserMessageCache")
                .error(parseHttpError);
        }
    }
})();
